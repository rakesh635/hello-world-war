pipeline {
    ...
    stages {
        stage('checkout') {
            steps {
                echo 'checkout'
                checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/rakesh635/hello-world-war.git']]])
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Deploy') {
            steps {
                sh 'curl --upload-file target/hello-world-war-1.0.0-SNAPSHOT.war "http://tomcat:password@35.200.166.99:8081/manager/text/deploy?path=/hello2&update=true"'
